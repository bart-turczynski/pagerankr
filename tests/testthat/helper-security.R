...
# reports nothing" (SEOR-fftbjnpl, porting seor's SEOR-fkvlzltx; the route is
# SEOR-feqgnbwt (a)). For pagerankr the two contracts coincide *today*:
# measured 2026-09-24 with live OSS Index credentials, the hard dependency
# closure (`igraph`, `rurl` and what they pull in) is 24 packages and reports
# zero advisories, so there is nothing to dispose of and the list is empty.
#
# Rows are deliberately NOT copied in from sitemapr ahead of need
# (SEOR-sxvcbuia): rule B below fails a row that is not currently reported, so
# an anticipatory row is stale on arrival and would fail the gate on the day it
# is added. The machinery is what is portable; the rows are not. Adding one
# when an advisory does appear is a one-block edit to `oss_index_allowlist`.
#
