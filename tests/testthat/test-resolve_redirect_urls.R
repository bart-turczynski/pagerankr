# Tests for resolve_redirect_urls()

describe("resolve_redirect_urls basic functionality", {
  it("resolves simple redirects", {
    redirects <- data.frame(
      from = c("A", "B"),
      to = c("B", "Final")
    )
    result <- resolve_redirect_urls(c("A", "B"), redirects)
  ...
  it("leaves non-redirected URLs unchanged", {
    ...
    result <- resolve_redirect_urls(c("A", "X", "Y"), redirects)
  ...
  it("resolves multi-hop chains", {
    ...
    result <- resolve_redirect_urls("A", redirects)
    expect_equal(result$resolved, "Final")
  })

  it("preserves NA inputs", {
    redirects <- data.frame(from = "A", to = "B")
    result <- resolve_redirect_urls(c("A", NA), redirects)
  ...
  it("handles empty URL vector", {
    redirects <- data.frame(from = "A", to = "B")
    result <- resolve_redirect_urls(character(0), redirects)
...
describe("resolve_redirect_urls policy passthrough", {
  it("errors on conflicting redirects with strict policy", {
    redirects <- data.frame(
      from = c("A", "A"),
      to = c("B", "C")
    )
    expect_error(resolve_redirect_urls("A", redirects), "Ambiguous redirect")
  })
  ...
  it("errors on loops with error policy", {
    ...
    expect_error(
      resolve_redirect_urls("A", redirects),
...
describe("resolve_redirect_urls input validation", {
  it("errors on non-character urls", {
    redirects <- data.frame(from = "A", to = "B")
    expect_error(
      resolve_redirect_urls(123, redirects),
      "must be a character vector"
    )
  })

  it("errors on non-data-frame redirects", {
    expect_error(resolve_redirect_urls("A", "not a df"), "must be a data frame")
  })

  it("errors on missing columns", {
    redirects <- data.frame(x = "A", y = "B")
    expect_error(resolve_redirect_urls("A", redirects), "must have")
  })
})


describe("resolve_redirect_urls empty results after preprocessing", {
...
describe("resolve_redirect_urls self-referencing handling", {
