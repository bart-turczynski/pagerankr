...
get_unique_edges <- function(edge_list_df,
  ...
  if (nrow(edge_list_df) == 0) {
    ...
    # This is particularly important if from_col or to_col are not the
    # defaults "from", "to" and edge_list_df was truly empty (0 rows, 0 cols
    # initially). However, the check above ensures columns exist if nrow > 0.
    # If nrow == 0, we assume it has the right structure or we return an empty
    # df with correct names.
    if (ncol(edge_list_df) == 0 && (from_col != "from" || to_col != "to")) {
      # This case is tricky: if edge_list_df is an empty data.frame() and
      # from_col/to_col are non-default it's better to return an empty df with
      # the right structure.
      ...
      return(empty_df)
    } else if (!all(c(from_col, to_col) %in% names(edge_list_df)) &&
                 ncol(edge_list_df) > 0) {
      # Edge list has columns, but not the ones we need.
      # This should have been caught by the validation above if nrow > 0.
      # If nrow == 0, but edge_list_df has other columns, we should still make
      # sure return has the right ones.
      # For safety, if columns are missing and it's empty, reconstruct.
      # However, the above validation (nrow > 0 && !all(...)) makes this less
      # likely.
      # If it's an empty df with wrong columns, this is a problem for
      # subsetting later.
      # Let's assume if nrow(edge_list_df) == 0, it either has correct columns
      # or is df().
    }
    # Return as is if empty and columns are assumed OK or it's truly empty.
    return(edge_list_df)
  }
