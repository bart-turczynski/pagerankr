...
#' `\lambda_i` is the per-source trust placed in the empirical data.
#'
#' ## Sample-size-dependent shrinkage (`lambda_i`)
#'
#' By default `\lambda_i = n_i / (n_i + k)`, the standard
...
#' `\lambda_i < 1` for any sampled source, hence a non-zero
#' `(1 - \lambda_i)` weight on every crawled link (see *Guarantees*).
#'
...
#'   data): `\lambda_i = 0`, so `P(i -> .)` is the pure structural prior. The
#'   crawl link still receives mass.
#' - **No structural prior** (observed transition whose source has no crawled
#'   out-links): `\lambda_i = 1`, so `P(i -> .)` is the pure empirical
#'   distribution — there is nothing to shrink toward.
#' - **Insufficient support** (`0 < n_i < min_support`): `\lambda_i = 0`. The
#'   empirical sample is treated as too small to trust, and the source falls
#'   back to its structural prior (when one exists).
#' - **Otherwise**: `\lambda_i = n_i / (n_i + k)` (or `lambda_fn(n_i)`).
#'
#' ## Edge universe
#'
...
#' `count_col` — the `n_i` totals and `\lambda_i` then reflect effective sample
...
#' `prior(i -> j) > 0`, and either `\lambda_i = 0` (pure prior) or
#' `\lambda_i < 1` (since `n_i / (n_i + k) < 1`), so the `(1 - \lambda_i)`
...
#'   controlling shrinkage strength in `\lambda_i = n_i / (n_i + k)`. Larger `k`
...
#'   the structural prior (`\lambda_i = 0`). Default `0` (no minimum).
