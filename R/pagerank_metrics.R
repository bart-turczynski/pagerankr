...
#' @describeIn pagerank_metrics Gini coefficient (0 = perfectly equal, 1 = maximally concentrated).
...
pr_gini <- function(x) {
  x <- as.numeric(x)
  x <- x[!is.na(x)]
  if (length(x) == 0 || all(x == 0)) return(NA_real_)
  n <- length(x)
  x_sorted <- sort(x)
  (2 * sum(seq_len(n) * x_sorted)) / (n * sum(x_sorted)) - (n + 1) / n
}

#' @describeIn pagerank_metrics Shannon entropy (higher = more uniform distribution).
#' @param x Numeric vector of non-negative values (typically PageRank scores).
...
pr_top_k_share <- function(x, k = 0.1) {
  ...
  x <- x[!is.na(x)]
  if (length(x) == 0 || all(x == 0)) return(NA_real_)
  if (k <= 0 || k > 1) stop("`k` must be between 0 (exclusive) and 1 (inclusive).", call. = FALSE)
  n <- length(x)
