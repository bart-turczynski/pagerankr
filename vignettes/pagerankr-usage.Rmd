...
Google splits PageRank across all outgoing links, including nofollow.
The nofollow share simply vanishes (the "evaporate" model). `pagerankr`
...
A `noindex` page cannot pass PageRank because it's not in Google's index.
All outgoing links from a noindex page are treated as nofollow:
...
pr <- pagerank(edges, indexability_df = idx_status,
               clean_edge_urls = FALSE)
print(pr)
```
...
pr_trap <- pagerank(edges, indexability_df = idx_status,
                    robots_blocked_action = "trap",
                    clean_edge_urls = FALSE)
print(pr_trap)

# "vanish": blocked page removed from results
pr_vanish <- pagerank(edges, indexability_df = idx_status,
...
impact_redirect <- simulate_changes(
  extended_links,
  add_redirects_df = data.frame(from = "OldPage", to = "About",
                                stringsAsFactors = FALSE),
  clean_edge_urls = FALSE
)
